...
if simulationInterface is None:
    ...
    if not(aircraftInterface is None):
        ...
        if not(curAircraft is None):
            flight_plan_name_array = curAircraft.getFlight_plan_waypoint_name_array()
            for j in range(0, len(flight_plan_name_array)) :
                print "SWA1897 flight_plan_name = ", flight_plan_name_array[j]
            print ""


    simulationInterface.setupSimulation(19200, 1)
    ...
    while True:
        ...
        print "status = ", server_runtime_sim_status
    ...
    #simulationInterface.write_trajectories("output_trajectory_" + str(millis) + ".xml")
