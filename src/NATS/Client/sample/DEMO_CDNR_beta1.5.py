...
NATS_SERVER_DIR = 'PLEASE_ENTER_NATS_SERVER_LOCATION_HERE'
...
if not (os.path.isdir(NATS_SERVER_DIR)) :
    ...
    for flag in CDNR_FLAG:
        ...
        if output_dirname == "":
            ...
            os.makedirs(NATS_SERVER_DIR + "/" + output_dirname)
        ...
        copyfile(NATS_SERVER_DIR + "/" + output_filename, NATS_SERVER_DIR + "/" + output_dirname + "/" + output_filename)


# Close connection from NATS Server
natsClient.disConnect()

if not os.path.exists(NATS_SERVER_DIR) :
    print "Directory NATS_SERVER_DIR does not exist"
else :
    # Notice!!!!
    # The following statements assume that NATS_Client and NATS_Server are installed on the same machine.
    # If NATS_Server is installed on another machine, the following Python codes will not work due to inability to access the server files.
    post_process_CDNR(NATS_SERVER_DIR + "/" + output_dirname);

    # Delete temp directory
    print "Deleting directory:", output_dirname
    rmtree(NATS_SERVER_DIR + "/" + output_dirname)
