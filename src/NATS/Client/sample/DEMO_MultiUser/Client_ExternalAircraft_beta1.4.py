...
XPLANE_PATH = "PLEASE_SPECIFY_PATH"
...
if not os.path.exists(XPLANE_PATH) :
    ...
    flagProfileCreated = False

    natsClient.login("user2")
        ...
        if (not(traj_ac_id is None) and (traj_ac_id != '')) :
            ...
            while (cnt_loop < max_cnt_loop):
                ...
                if (not(tmpState is None)) :
                    while ((array_index < (len(xPlaneData) - 1)) or (len(tmpState) != num_valid_state) or (tmpState[0].startswith('  _real,_time '))):
                        array_index = array_index + 1
                ...
                if (len(currentState) == num_valid_state) :
