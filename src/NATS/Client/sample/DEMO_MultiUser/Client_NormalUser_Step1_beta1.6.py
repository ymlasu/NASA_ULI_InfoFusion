...
if simulationInterface is None:
    ...
    # List all assigned ground vehicles of the current logined user
    array_assignedGroundVehicles = groundVehicleInterface.getAssignedGroundVehicleIds()
    if not(array_assignedGroundVehicles is None) :
        for i in range(0, len(array_assignedGroundVehicles)) :
            print "User assigned groundVehicles = ", array_assignedGroundVehicles[i]
    ...
    # Setting new state for ground vehicle
    time.sleep(30)

    curGroundVehicle = groundVehicleInterface.select_groundVehicle('BELT123')
    if not(curGroundVehicle is None):
        print "Setting new state of ground vehicle BELT123"
        groundOperatorInterface.setVehicleContact('BELT123')
