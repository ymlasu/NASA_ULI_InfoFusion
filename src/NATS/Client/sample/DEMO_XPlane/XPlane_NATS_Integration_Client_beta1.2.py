# This program read data file generated from XPlan simulator and send it to NATS Server
...
if simulationInterface is None:
    ...
    time_step = 1 # sec
    cnt_loop = 0
    max_cnt_loop = 10 # Number of loop to be processed
    
    array_index = -1
    
    xPlaneDataFile = open("sample/DEMO_XPlane/XPlane_Data_beta_1.1.txt")
    ...
    while (cnt_loop < max_cnt_loop):
        ...
        currentState = xPlaneData[array_index]
        
        while ((len(currentState) != 32) or (currentState[0].startswith('  _real,_time '))):
            array_index = array_index + 1
            currentState = xPlaneData[array_index]
